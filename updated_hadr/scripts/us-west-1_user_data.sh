...
s3fs hemtestbucket-hadr-us-west-1 /var/www/html/app -o iam_role=ec2_dynamo_s3_access_role -o allow_other
...
<head>
    ...
    <title>Customer Management - us-west-1 </title>
    <script>
        async function insertData() {
            ...
            try {
                const response = await fetch('http://172.31.24.48:5000/insert', {
                    method: 'POST',
                    headers: {
        ...
        async function fetchData() {
            try {
                const response = await fetch('http://172.31.24.48:5000/read', {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    }
...
<body>
    <h1>Customer Management - us-west-1</h1>
...
# Initialize DynamoDB
dynamodb = boto3.resource('dynamodb', region_name='us-west-1')  # Use primary region here
